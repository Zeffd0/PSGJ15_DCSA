    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                float3 ro = float3(0.0, 3.0 * sin(_Time.y * 0.4) * cos(_Time.y * 0.4), 3.0 * (1.0 - sin(_Time.y * 0.4) * cos(_Time.y * 0.4)));
                ...
                col = lerp(col, dot(col, float3(0.33, 0.33, 0.33)), -0.5);
                col *= 0.5 + 0.5 * pow(16.0 * q.x * q.y * (1.0 - q.x) * (1.0 - q.y), 0.7);

                return fixed4(col, smoothstep(0.55, 0.76, 1.0 - res.x / 5.0));
            }
            ENDCG
        }
    }
